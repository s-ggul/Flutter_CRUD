...
class DBHelper {
  ...
  Future<Database> get database async {
    ...
    _db = openDatabase(
      ...
      // 각 플랫폼 별로 경로가 제대로 생성됐는지 보장할 수 있는 가장 좋은 방법
      ...
      // 버전을 설정.
      // onCreate 함수에서 수행되며 데이터베이스 업그레이드와 다운그레이드를
      // 수행하기 위한 경로를 제공
      version: 1,
    );
    return _db;
  ...
  Future<void> insertMemo(Memo memo) async {
    ...
    // Memo를 올바른 테이블에 추가
    // 또한 `conflictAlgorithm`을 명시
    // 본 예제에서는 만약 동일한 memo가 여러번 추가되면, 이전 데이터를 덮어쓸 것입니다.
    await db.insert(
      TableName,
      memo.toMap(),
